    ...
    .login(:style='`background-image: url(` + bgUrl + `);`')
      .login-sd
        .d-flex.mb-5
          .login-logo
            v-avatar(tile, size='34')
              v-img(:src='logoUrl')
          .login-title
            .text-h6.grey--text.text--darken-4 {{ 'Yacaré Documentación'}}
        v-alert.mb-0(
          v-model='errorShown'
        ...
        template(v-if='screen === `login` && selectedStrategy.strategy.useForm')
          .login-subtitle
            .text-subtitle-1 {{$t('ingresa tus datos')}}
          .login-form
            v-text-field(
              solo
              flat
            ...
            .text-center.mt-5
              v-btn.text-none(
                ...
                href='#forgot'
                ): .caption {{ $t('¿Olvidaste tu contraseña?') }}
              v-btn.text-none(
  ...
  methods: {
    ...
    async login () {
      ...
      if (this.username.length < 2) {
        this.errorMessage = this.$t('Email no valido')
        this.errorShown = true
        this.$refs.iptEmail.focus()
      } else if (this.password.length < 2) {
        this.errorMessage = this.$t('Password no valido')
        this.errorShown = true
        this.$refs.iptPassword.focus()
      } else {
        this.loaderColor = '#38B000'
...
<style lang="scss">
  .login {
   background-color:#38B000;  //cambios realizado por EFM(26/10/2022)
